...
FIXED_SAMPLING_ROOT = f"Data/FixedSamplingResults/{MODEL_RUN_NAME}/FixedSampling"
...
SUMMARY_CSV_PATH = f"Data/FixedSamplingResults/{MODEL_RUN_NAME}_fixed_sampling_summary.csv"
SUMMARY_CSV_FIELDS = [
...
class TestSimulation():
    ...
    def run_testing(self):
        ...
        for i in range(self.n_test_laps):
            ...
            while not observation['colision_done'] and not observation['lap_done']:
                # logging only: the pre-step state/whether this step's scan is
                # fresh, captured before self.get_navigation_observation()/
                # self.planner.plan()/self.run_step() run exactly as before.
                control_step_index = self.control_step
                pre_step_state = observation['state']
                self.prev_obs = observation

                observation_for_navigation = self.get_navigation_observation(observation)
                lidar_fresh = self.last_step_was_fresh
                action = self.planner.plan(observation_for_navigation)
                observation = self.run_step(action)
                if SHOW_TEST: self.env.render('human_fast')

                # logging only, derived from the just-produced `observation`;
                # does not feed back into planning/action selection.
                self.lap_progress = max(self.lap_progress, self.progress_track.calculate_progress_percent(observation['state'][0:2]))
                self.cumulative_reward += observation['reward']
                ...
                self.step_rows.append({
                    ...
                    "commanded_steering": action[0],
                    "commanded_speed": action[1],
                    "lidar_fresh": lidar_fresh,
                    "instantaneous_reward": observation['reward'],
    ...
    def get_navigation_observation(self, observation):
        """Sample-and-hold gate: choose the current or cached LiDAR scan.

        Everything else in `observation` passes through untouched; only the
        copy handed to the planner has its 'scan' field replaced, so
        collision/lap-done/lap-timing logic downstream keeps seeing the real,
        fresh observation every control step.
        """
        ...
        navigation_observation['scan'] = self.last_fresh_scan
